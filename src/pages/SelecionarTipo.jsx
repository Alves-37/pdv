...
export default function SelecionarTipo() {
  ...
  async function onSelectTenant(id) {
    ...
    setTimeout(() => setSelectingTenantId(''), 900)
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      {selectingTenantId && (
        <div className="text-sm text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-md px-3 py-2 flex items-center gap-2">
          ...
          <span>Selecionando negócio...</span>
        </div>
      )}
              ...
              >
                ...
                <div className="mt-3">
                  <button
                    ...
                    disabled={Boolean(selectingTenantId) || isSelected}
                  >
                    {isSelected ? 'Selecionado' : (selectingTenantId === t.id ? 'Selecionando...' : 'Selecionar')}
                  </button>
                </div>
                ...
                <div className="mt-3 grid grid-cols-4 gap-2">
                  ...
                  <button
                    className="btn-outline flex items-center justify-center gap-2 text-amber-700 border-amber-200 hover:bg-amber-50"
                    onClick={() => onResetTenant(t)}
      ...
      <ConfirmDialog
        open={resetOpen}
        title="Resetar dados do negócio"
        message={resetTenant ? `Isto vai apagar os dados deste negócio (produtos, vendas, clientes, usuários, etc.). Tenant: ${resetTenant.nome || resetTenant.id}` : ''}
        confirmText="Resetar"
        cancelText="Cancelar"
        loading={resetSubmitting}
        danger
        onCancel={() => { if (!resetSubmitting) { setResetOpen(false); setResetTenant(null) } }}
        onConfirm={async () => {
          if (!resetTenant?.id) return
          try {
            setResetSubmitting(true)
            await api.resetDadosTenant(resetTenant.id)
            setResetOpen(false)
            setResetTenant(null)
          } catch (e) {
            setError(e.message)
